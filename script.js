  ...
  pollingInterval = setInterval(async function () {
    try {
      ...
      if (getFileDataResult && getFileDataResult.data) {
        ...
        const formattedActionsItemList = formatActionItems(data.action_items.S);
        const formattedCustomerData = formatCustomerResponse(
        ...
        ulElement.className = 'action-items-list';

        const actionItemsContainer =
          extractedDataDiv.querySelector('p:nth-of-type(3)');

        formattedActionsItemList.forEach((item) => {
          const li = document.createElement('li');
          li.textContent = item;
          ulElement.appendChild(li);
        });

        actionItemsContainer.appendChild(ulElement);

        // Update Customer Data

        const customerDataContainer =
          extractedDataDiv.querySelector('p:nth-of-type(4)');

        const ul = document.createElement('ul');
        ul.className = 'customer-data-list';

        formattedCustomerData.forEach(({ key, value }) => {
          const li = document.createElement('li');
        ...
        customerDataContainer.appendChild(ul);
...
// async function pollForData() {
//   const maxRetries = 0; // Number of retries before returning the fake data response
...
function formatActionItems(actionItems) {
  if (!actionItems || typeof actionItems !== 'string') {
    return [];
  }

  const formattedItems = actionItems
    .split('\n')
...
function formatCustomerResponse(response) {
